...
## Production: GitHub Pages (current)
...
runs in `--safe` mode, which forbids custom `_plugins/`. This site has three, and
`_plugins/datapage_generator.rb` generates the 59 `/languages/:code/` and 7
`/styles/:slug/` pages from `_data/`. Under the sandboxed build those 66 pages simply
would not exist — measured: `--safe` yields 88 HTML files instead of 153, and exits 0.
...
### Closed: `_redirects` does nothing on GitHub Pages
...
via `jekyll-redirect-from`, which works anywhere — 39 legacy URLs in total, listed in
...
## Production: Cloudflare Pages (an option, not set up)

Worth considering for native `_redirects`, an arbitrary build command, and per-PR
preview URLs. **No project exists** — that needs Cloudflare dashboard access.
...
Two things to confirm on the first preview build:

1. **Ruby version.** Cloudflare's build image reads `.ruby-version`, but ships a
   fixed set of Ruby builds. If 3.3.6 is unavailable, either pin to the nearest
   version Cloudflare offers (and re-resolve `Gemfile.lock`) or set the
   `RUBY_VERSION` build environment variable.
2. **`_redirects` is served.** Jekyll skips underscore-prefixed files, so
   `_config.yml` names it explicitly under `include:`. Confirm `_site/_redirects`
   exists in the build output and that `/schema` 301s to `/docs/schema/`.

If Cloudflare becomes the production host, `CNAME` becomes inert but harmless —
leave it, so GitHub Pages remains a working fallback.
...
| Path | Produced by |
|---|---|
| `sitemap.xml` | `jekyll-sitemap` — 107 URLs, trailing slashes matching `permalink: pretty` |
...
| 39 legacy redirect stubs | `redirect_from:` / `redirect_to:` front matter, listed in `script/redirects.tsv` |

153 HTML routes total: 114 pages and 39 redirect stubs, one per row of `script/redirects.tsv`.

---

## Visual regression

There is none, deliberately. A computed-style comparison harness was used while the
rewrite was being built and then removed: it existed to prove the new markup matched
the old, and that question is settled. Keeping it would have meant keeping npm, a
`package.json` and a multi-megabyte snapshot in a repo whose entire premise is that
the build is `bundle exec jekyll build` and nothing else.

If a future change to `_sass/` or `_layouts/` warrants that kind of net again, it has
to be rebuilt — the harness is not recoverable from this repository's history.
...
**Build succeeds but 66 pages are missing.** Custom `_plugins/` did not run —
