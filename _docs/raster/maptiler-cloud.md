...
MapTiler team provides a **free (for non-commercial use), highly reliable maps API** with map styles based on the **OpenMapTiles schema** and powered by **OpenStreetMap data**. The entire planet is updated weekly, with both high-performance vector and raster tiles available instantly to developers and companies.
